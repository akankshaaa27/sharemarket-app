...
export default function ClientProfiles() {
  ...
  async function onOpenEdit(profile) {
    try {
      ...
      const shareHoldings = full.companies && Array.isArray(full.companies) ? full.companies : (full.shareHoldings || []);
      setEditing({
        ...full,
        shareHoldings: shareHoldings.map(h => ({
          ...
          review: h.review || { status: "pending", notes: "", reviewedAt: "", reviewedBy: "" },
        })),
      });
    } catch (e) {
      setError(e.message);
  ...
  function addEditingShareHolding() {
    setEditing(e => ({ ...e, shareHoldings: [...(e?.shareHoldings || []), emptyHolding()] }));
  }
  function removeEditingShareHolding(index) {
    setEditing(e => ({ ...e, shareHoldings: e.shareHoldings.filter((_, i) => i !== index) }));
  }
  function updateEditingShareHolding(index, field, value) {
    setEditing(e => ({ ...e, shareHoldings: e.shareHoldings.map((h, i) => i===index ? { ...h, [field]: value } : h) }));
  }
  ...
  function saveReviewFor(index, status, notes) {
    setEditing(e => ({
      ...e,
      shareHoldings: e.shareHoldings.map((h, i) => i===index ? {
        ...h,
        review: {
          status,
          notes,
            ...
            {items.map((p)=> (
              <tr key={p._id} className="border-t">
                ...
                <td className="p-2 text-center space-x-3">
                  ...
                  <button onClick={()=>onOpenEdit(p)} className="text-blue-600 hover:underline">Edit</button>
          ...
          <div className="absolute inset-0 p-4 overflow-y-auto">
            <div className="mx-auto max-w-5xl bg-card text-foreground rounded-lg border shadow-lg">
              <div className="p-4 border-b flex items-center justify-between">
                ...
                <button onClick={()=>!savingEdit && setEditing(null)} className="text-sm px-2 py-1 rounded border">Close</button>
              </div>
              <div className="p-4 space-y-6">
                <div className="grid grid-cols-2 gap-4 p-4 border rounded-lg">
                  ...
                  <div className="col-span-2 grid grid-cols-3 gap-3">
                    <div className="space-y-1">
                      <label className="text-sm">Name 1 *</label>
                      <input className="w-full border rounded p-2" value={editing.shareholderName?.name1||""} onChange={(e)=>setEditing(ed=>({...ed, shareholderName:{...(ed.shareholderName||{}), name1:e.target.value}}))} />
                    </div>
                    <div className="space-y-1">
                      <label className="text-sm">Name 2</label>
                      <input className="w-full border rounded p-2" value={editing.shareholderName?.name2||""} onChange={(e)=>setEditing(ed=>({...ed, shareholderName:{...(ed.shareholderName||{}), name2:e.target.value}}))} />
                    </div>
                    <div className="space-y-1">
                      <label className="text-sm">Name 3</label>
                      <input className="w-full border rounded p-2" value={editing.shareholderName?.name3||""} onChange={(e)=>setEditing(ed=>({...ed, shareholderName:{...(ed.shareholderName||{}), name3:e.target.value}}))} />
                    </div>
                  </div>
                  <div className="space-y-1">
                    <label className="text-sm">PAN *</label>
                    <input className="w-full border rounded p-2" value={editing.panNumber||""} onChange={(e)=>setEditing(ed=>({...ed, panNumber:e.target.value.toUpperCase()}))} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-sm">Aadhaar Number</label>
                    <input className="w-full border rounded p-2" value={editing.aadhaarNumber||""} onChange={(e)=>setEditing(ed=>({...ed, aadhaarNumber:e.target.value.replace(/\D/g,'').slice(0,12)}))} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-sm">Demat Account Number</label>
                    <input className="w-full border rounded p-2" value={editing.dematAccountNumber||""} onChange={(e)=>setEditing(ed=>({...ed, dematAccountNumber:e.target.value}))} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-sm">Demat Account Created With</label>
                    <input className="w-full border rounded p-2" value={editing.dematCreatedWith||""} onChange={(e)=>setEditing(ed=>({...ed, dematCreatedWith:e.target.value}))} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-sm">DMAT Account Created By (Person)</label>
                    <input className="w-full border rounded p-2" value={editing.dematCreatedWithPerson||""} onChange={(e)=>setEditing(ed=>({...ed, dematCreatedWithPerson:e.target.value}))} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-sm">Creator Contact Number</label>
                    <input className="w-full border rounded p-2" value={editing.dematCreatedWithPersonNumber||""} onChange={(e)=>setEditing(ed=>({...ed, dematCreatedWithPersonNumber:e.target.value.replace(/\D/g,'').slice(0,15)}))} />
                  </div>
                  <div className="col-span-2 space-y-1">
                    <label className="text-sm">Address</label>
                    <input className="w-full border rounded p-2" value={editing.address||""} onChange={(e)=>setEditing(ed=>({...ed, address:e.target.value}))} />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 p-4 border rounded-lg">
                  ...
                  <div className="space-y-1">
                    <label className="text-sm">Bank Number</label>
                    <input className="w-full border rounded p-2" value={editing.bankDetails?.bankNumber||""} onChange={(e)=>setEditing(ed=>({...ed, bankDetails:{...(ed.bankDetails||{}), bankNumber:e.target.value}}))} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-sm">Branch</label>
                    <input className="w-full border rounded p-2" value={editing.bankDetails?.branch||""} onChange={(e)=>setEditing(ed=>({...ed, bankDetails:{...(ed.bankDetails||{}), branch:e.target.value}}))} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-sm">Bank Name</label>
                    <input className="w-full border rounded p-2" value={editing.bankDetails?.bankName||""} onChange={(e)=>setEditing(ed=>({...ed, bankDetails:{...(ed.bankDetails||{}), bankName:e.target.value}}))} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-sm">IFSC Code</label>
                    <input className="w-full border rounded p-2" value={editing.bankDetails?.ifscCode||""} onChange={(e)=>setEditing(ed=>({...ed, bankDetails:{...(ed.bankDetails||{}), ifscCode:e.target.value.toUpperCase()}}))} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-sm">MICR Code</label>
                    <input className="w-full border rounded p-2" value={editing.bankDetails?.micrCode||""} onChange={(e)=>setEditing(ed=>({...ed, bankDetails:{...(ed.bankDetails||{}), micrCode:e.target.value}}))} />
                  </div>
                </div>
                ...
                <div className="p-4 border rounded-lg">
                  <div className="flex justify-between items-center mb-4">
                    <h3 className="font-semibold">Share Holdings</h3>
                    <button type="button" onClick={addEditingShareHolding} className="px-3 py-1 rounded border">Add Company</button>
                  </div>

                  {editing.shareHoldings?.map((h, idx) => (
                    <div key={idx} className="grid grid-cols-2 gap-4 p-4 mb-4 border rounded-lg relative">
                      <button type="button" onClick={()=>removeEditingShareHolding(idx)} disabled={editing.shareHoldings.length===1} className="absolute top-2 right-2 text-sm px-2 py-1 rounded border">Remove</button>
                      <div className="col-span-2 font-medium text-sm text-muted-foreground">Company #{idx+1}</div>

                      <div className="space-y-1">
                        <label className="text-sm">Company Name *</label>
                        <input className="w-full border rounded p-2" value={h.companyName} onChange={(e)=>updateEditingShareHolding(idx,'companyName',e.target.value)} />
                      </div>

                      <div className="space-y-1">
                        <label className="text-sm">ISIN Number *</label>
                        <input className="w-full border rounded p-2" value={h.isinNumber} onChange={(e)=>updateEditingShareHolding(idx,'isinNumber',e.target.value.toUpperCase())} />
                      </div>

                      <div className="space-y-1">
                        <label className="text-sm">Folio Number</label>
                        <input className="w-full border rounded p-2" value={h.folioNumber||""} onChange={(e)=>updateEditingShareHolding(idx,'folioNumber',e.target.value)} />
                      </div>

                      <div className="space-y-1">
                        <label className="text-sm">Certificate Number</label>
                        <input className="w-full border rounded p-2" value={h.certificateNumber||""} onChange={(e)=>updateEditingShareHolding(idx,'certificateNumber',e.target.value)} />
                      </div>

                      <div className="space-y-1">
                        <label className="text-sm">Quantity *</label>
                        <input type="number" className="w-full border rounded p-2" value={h.quantity||0} onChange={(e)=>updateEditingShareHolding(idx,'quantity',parseInt(e.target.value)||0)} />
                      </div>

                      <div className="space-y-1">
                        <label className="text-sm">Face Value *</label>
                        <input type="number" step="0.01" className="w-full border rounded p-2" value={h.faceValue||0} onChange={(e)=>updateEditingShareHolding(idx,'faceValue',parseFloat(e.target.value)||0)} />
                      </div>

                      <div className="space-y-1">
                        <label className="text-sm">Distinctive From</label>
                        <input className="w-full border rounded p-2" value={h.distinctiveNumber?.from||""} onChange={(e)=>updateEditingShareHolding(idx,'distinctiveNumber',{...(h.distinctiveNumber||{}), from:e.target.value})} />
                      </div>

                      <div className="space-y-1">
                        <label className="text-sm">Distinctive To</label>
                        <input className="w-full border rounded p-2" value={h.distinctiveNumber?.to||""} onChange={(e)=>updateEditingShareHolding(idx,'distinctiveNumber',{...(h.distinctiveNumber||{}), to:e.target.value})} />
                      </div>

                      <div className="space-y-1">
                        <label className="text-sm">Purchase Date</label>
                        <input type="date" className="w-full border rounded p-2" value={h.purchaseDate||""} onChange={(e)=>updateEditingShareHolding(idx,'purchaseDate',e.target.value)} />
                      </div>
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button type="button" className="px-2 py-1 text-sm rounded border" onClick={()=>setShowReviewDialog(idx)}>Review</button>
                        </div>
                      </div>
                ...
                <div className="grid grid-cols-2 gap-4 p-4 border rounded-lg">
                  ...
                  <div className="space-y-1">
                    <label className="text-sm">Current Date</label>
                    <input type="date" className="w-full border rounded p-2" value={editing.currentDate? String(editing.currentDate).slice(0,10):""} onChange={(e)=>setEditing(ed=>({...ed, currentDate:e.target.value}))} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-sm">Status</label>
                    <select className="w-full border rounded p-2" value={editing.status||"Active"} onChange={(e)=>setEditing(ed=>({...ed, status:e.target.value}))}>
                      {['Active','Closed','Pending','Suspended'].map(s => <option key={s} value={s}>{s}</option>)}
                    </select>
                  </div>
                  <div className="col-span-2 space-y-1">
                    <label className="text-sm">Remarks</label>
                    <input className="w-full border rounded p-2" value={editing.remarks||""} onChange={(e)=>setEditing(ed=>({...ed, remarks:e.target.value}))} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-sm">Dividend Amount</label>
                    <input type="number" className="w-full border rounded p-2" value={editing.dividend?.amount ?? 0} onChange={(e)=>setEditing(ed=>({...ed, dividend:{...(ed.dividend||{}), amount:Number(e.target.value)}}))} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-sm">Dividend Date</label>
                    <input type="date" className="w-full border rounded p-2" value={editing.dividend?.date ? String(editing.dividend.date).slice(0,10):""} onChange={(e)=>setEditing(ed=>({...ed, dividend:{...(ed.dividend||{}), date:e.target.value}}))} />
                  </div>
                </div>

                <div className="flex justify-end gap-3 pt-4 border-t">
                  <button type="button" onClick={()=>setEditing(null)} className="px-4 py-2 rounded border">Cancel</button>
                  <button onClick={onSaveEdit} disabled={savingEdit || !editing.shareholderName?.name1 || !editing.panNumber} className="bg-primary text-primary-foreground px-4 py-2 rounded disabled:opacity-50">{savingEdit?"Saving...":"Save Changes"}</button>
                </div>
              </div>
            </div>
          </div>
                      ...
                      return (
                        <>
                          ...
                          <div className="space-y-1">
                            ...
                            <select className="w-full border rounded p-2" value={holding.review?.status||"pending"} onChange={(e)=>updateEditingShareHolding(idx,'review',{...(holding.review||{}), status:e.target.value})}>
                              {reviewStatusOptions.map(opt => (<option key={opt.value} value={opt.value}>{opt.label}</option>))}
                            </select>
                          </div>
                          <div className="space-y-1">
                            <label className="text-sm">Review Notes</label>
                            <textarea className="w-full h-24 p-2 border rounded resize-none" value={holding.review?.notes||""} onChange={(e)=>updateEditingShareHolding(idx,'review',{...(holding.review||{}), notes:e.target.value})} />
                          </div>
                          {holding.review?.reviewedAt && (
                            <div className="text-xs text-muted-foreground">Last reviewed: {new Date(holding.review.reviewedAt).toLocaleString()} by {holding.review.reviewedBy}</div>
                          )}
                          <div className="flex justify-end gap-2 pt-2">
                            <button type="button" className="px-4 py-2 rounded border" onClick={()=>setShowReviewDialog(null)}>Cancel</button>
                            <button type="button" className="px-4 py-2 rounded bg-primary text-primary-foreground" onClick={()=>saveReviewFor(idx, editing.shareHoldings[idx].review?.status||"pending", editing.shareHoldings[idx].review?.notes||"")}>Save Review</button>
                          </div>
                        </>
                      );
                    })()}
